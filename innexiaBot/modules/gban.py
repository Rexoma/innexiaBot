...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in SUDOERS:
        message.reply_text(
            "OOOH someone's trying to gban a SUDOERS! *grabs popcorn*",
        )
        return
    ...
    if EVENT_LOGS:
        ...
        send_to_list(bot,SUDOERS, log_message, html=True)
        ...
        try:
            ...
            if excp.message in GBAN_ERRORS:
                ...
                if EVENT_LOGS:
                    ...
                    send_to_list(
                        bot, SUDOERS, f"Could not gban due to: {excp.message}",
                    )
...
def ungban(update: Update, context: CallbackContext):
    ...
    if EVENT_LOGS:
        ...
        send_to_list(bot, SUDOERS, "un-gban complete!")
...
def gbanlist(update: Update, context: CallbackContext):
    ...
    with BytesIO(str.encode(json.dumps(data))) as output:
        output.name = "innexiaGbanned.json"
        update.effective_message.reply_document(
            document=output,
            filename="innexiaGbanned.json",
...
def __user_info__(user_id):
    ...
    if user_id == dispatcher.bot.id:
        return ""
    if int(user_id) in INSPECTOR + SUDOERS:
        return ""
    if is_gbanned:
